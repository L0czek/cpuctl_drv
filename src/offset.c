...
{
    u64 msr_value = (1ULL << 63) | ((plane & 0xf) << 40) | (1ULL << 36) | (0ULL << 32) | 0ULL;
...
{
    u64 msr_value = (1ULL << 63) | ((plane & 0xf) << 40) | (1ULL << 36) | (1ULL << 32) | (offset & 0xffffffff);
    ...
    for (i = 0; i < PLANES; ++i) {
        if ((1ULL << i) & offsets->what) {
            ...
            if (get_offset(i) != offsets->values[i]) {
                printk(KERN_INFO "cpuctl error setting offset for plane: %lld\n", i);
                return -EFAULT;
            }
        }
    }
